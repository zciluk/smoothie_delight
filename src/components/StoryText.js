...
class StoryText extends Component {
  ...
  componentDidMount() {
    ...
    setInterval(() => {
      ...
      if (this.state.typingMode === "delete") {
        this.deleteKeyword();
      } else if (this.state.typingMode === "put") {
        this.putKeyword(this.state.keywordsStack[0]);
      } else if (this.state.keyword === "") {
        this.setState({
          typingMode:
            this.state.typingMode + "put"[this.state.typingMode.length]
        });
      } else if (this.state.keyword !== "") {
        this.setState({
          typingMode:
            this.state.typingMode + "delete"[this.state.typingMode.length]
        });
      }
    }, 250);
  }
  deleteKeyword() {
    this.state.keyword === ""
      ? this.setState({
          keywordsStack: this.state.keywordsStack.slice(1),
          typingMode: ""
        })
      : this.setState({ keyword: this.state.keyword.slice(0, -1) });
  }
  putKeyword(word) {
    word.length === this.state.keyword.length
      ? this.setState({
          typingMode: "",
          keywordsStack: [...this.state.keywordsStack, word]
        })
      : this.setState({
          keyword: this.state.keyword + word[this.state.keyword.length]
        });
  }
  render() {
    ...
    return (
      <MainText>
        Eat healthy, have{" "}
        <p>
          healthier <PurpleText>{keyword}</PurpleText>
          <BlinkingCursor pose={isVisible ? "visible" : "hidden"}>
            |
          </BlinkingCursor>
        </p>
      </MainText>
